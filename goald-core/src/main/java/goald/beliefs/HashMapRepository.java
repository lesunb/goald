...
public class HashMapRepository implements IRepository {
	...
	public Bundle queryForDefinition(Goal goal){
		...
		Set<Bundle> bundlesSet = getKnownBundles(BundleType.DEFINITION).get(goal.getIdentication());
		if(bundlesSet != null) {
	...
	public List<Bundle>  queryForImplementations(Goal goal){
		List<Bundle> bundles = new ArrayList<>();
		Set<Bundle> bundlesSet = getKnownBundles(BundleType.IMPLEMENTATION).get(goal.getIdentication());
		if(bundlesSet != null) {
			bundles.addAll(bundlesSet);
		}
		return bundles;
