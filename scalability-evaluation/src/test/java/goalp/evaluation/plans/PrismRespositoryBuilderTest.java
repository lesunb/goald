...
public class PrismRespositoryBuilderTest {
	...
	public void setup() {
		...
		PlanningExperimentBuilder expBuilder = ((PlanningExperimentBuilder) PlanningExperimentBuilder.create()
				.setName("teste repo").setResponseVariable("Time (ms)")
				// experiment evaluated factors
				.addFactor("Variability").addFactor("Plan size"))
						// params for creating the repository
						.putRepoSpec("depth", 4).putRepoSpec("numOfDependencies", 5)
						.putRepoSpec("variabilityRange", 4, 4).putRepoSpec("numberOfTrees", 3)
						.putRepoSpec("contextSpace", getContextSpace(20))
						.putRepoSpec("agentContext", getContextSpace(20)).putRepoSpec("contextVariabilityP", 5)
		...
		PrismRepositoryFactory.create().buildBySpec(experiment.getRepoSpec()).setSetupWithRepo(setup)
